      ...
      {editing ? (
        ...
        <div className="flex items-start gap-3">
          <button
            ...
            className={`mt-0.5 grid size-7 shrink-0 place-items-center rounded-lg border-2 transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent ${todo.completed ? "border-ink bg-ink text-white" : "border-[#9ba7a3] hover:border-ink"}`}
          >
            {todo.completed && <span aria-hidden>✓</span>}
          </button>
          <div className="min-w-0 flex-1">
            <p
              className={`font-bold leading-7 ${todo.completed ? "text-muted line-through decoration-2" : "text-ink"}`}
            >
              {todo.title}
            </p>
